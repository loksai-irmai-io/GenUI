...
const DataVisualizationWidget: React.FC<DataVisualizationWidgetProps> = ({
  ...
  if (isMinimized) {
    return (
      <div
        className={`enterprise-card p-4 cursor-pointer hover:bg-slate-700/30 transition-all duration-300 transform hover:scale-105 ${className}`}
      ...
      >
        ...
        <p className="text-sm text-slate-400 mt-2">Click to expand visualization</p>
      </div>
    );
  }
  ...
  return (
    <div
      className={`enterprise-card p-6 focus-visible:ring-2 focus-visible:ring-blue-400 outline-none transition-all duration-300 ${
        maximized ? "fixed inset-4 z-50 max-w-none animate-scale-in" : ""
      } ${className}`}
      tabIndex={0}
      aria-label={title}
    >
